...
export function Room() {
  ...
  async function handleLikeQuestion(questionId: string, likeId: string | undefined) {
    ...
    if (likeId) {
      ...
      await database.ref(`rooms/${params.id}/questions/${questionId}/likes`).push(like);
    ...
    <div id="page-room" className={theme}>
      ...
      <main className="content">
        <div className="room-title">
          <h1>Sala {room?.title} {room?.closed && '[ENCERRADA]'}
          </h1>
          {questions.length > 0 && <span>{questions.length} pergunta(s)</span>}
        </div>
        <form onSubmit={handleSendQuestion}>
          <textarea
            placeholder={!room?.closed ? "O que você quer perguntar?" : "Envio de perguntas encerrado"}
          ...
          <div className="form-footer">
            ...
            <Button type="submit" disabled={!user || room?.closed}>Enviar pergunta</Button>
          </div>
        </form>
